...
until "${compose[@]}" exec -T postgres pg_isready -U bot >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U bot -d bot \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/bot.sql"
