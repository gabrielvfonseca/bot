...
<html lang="en">
  <head><meta charset="utf-8"><title>Bot desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  const app = await electron.launch({
    ...
    env: {
      ...process.env,
      BOT_WEB_URL: `data:text/html;charset=utf-8,${encodeURIComponent(fixture)}`,
    },
  });
  ...
  try {
    ...
    await expect(page).toHaveTitle("Bot desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { botDesktop?: BotDesktop }).botDesktop;
