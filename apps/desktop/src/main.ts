...
import type { DesktopReachability, DesktopSetup } from "@bot/contracts";
import { LOCAL_SETTINGS_PAGE } from "@bot/contracts/local-settings";
import {
  app,
  BrowserWindow,
  dialog,
...
const PERFORMANCE_USER_DATA = process.env.BOT_PERFORMANCE_USER_DATA;
/** Test hook: where the app-managed stack answers. Mode `new` still requires loopback. */
const LOCAL_WEB_URL = process.env.BOT_LOCAL_WEB_URL?.trim() || DEFAULT_LOCAL_WEB_URL;
const PROBE_TIMEOUT_MS = 8_000;
const DESKTOP_STACK_PROBE_PATH = "/.well-known/bot-desktop-stack";
const DESKTOP_STACK_TOKEN_HEADER = "x-bot-desktop-stack-token";
...
const WARM_WINDOW_TTL_MS = warmWindowTtlMs(process.env.BOT_WARM_WINDOW_TTL_MS);

const updaterEnvironment = {
  packaged: app.isPackaged,
  version: app.getVersion(),
  disabled: process.env.BOT_DISABLE_AUTO_UPDATE === "1",
};
 ...
 * fixtures omit the Bot app-state marker.
 */
async function waitForMountedAppDocument(contents: Electron.WebContents) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await contents.executeJavaScript(`(() => {
      const appState =
        document.querySelector("[data-bot-app-state]")?.getAttribute("data-bot-app-state") ??
        null;
      ...
      const authOrWelcomeSurface = Boolean(
        document.querySelector('[data-bot-surface="welcome"]') ||
      ...
      // Desktop e2e fixtures mount a plain page without Bot app-state markers.
      if (appState === null) {
...
async function installBundledRenderer(
  ...
  if (!app.isPackaged || process.env.BOT_DISABLE_BUNDLED_RENDERER === "1") return;
...
function installApplicationMenu() {
  ...
  const changeServer: Electron.MenuItemConstructorOptions = {
    id: "change-bot-server",
    label: "Change Bot Server…",
...
/** Setup IPC must only answer the setup window, never a connected Bot server. */
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return {
        ...
        url,
        error: "That address redirects elsewhere. Enter the final Bot server address.",
      };
    }
    if (!response.ok) {
      return {
    ...
    if (!isBotHealth(health)) {
      return {
        ok: false,
        status: response.status,
        url,
        error: "That address did not respond like a Bot server.",
      };
    }
    return {
      ok: true,
...
app.whenReady().then(async () => {
  ...
  const target = resolveStartupTarget({
    envUrl: process.env.BOT_WEB_URL,
    saved: currentSetup,
    forceSetup: process.env.BOT_FORCE_SETUP === "1",
  });
  if (process.env.BOT_PERFORMANCE_CLEAR_CACHE === "1") {
    ...
    try {
      ...
      if (setup.mode === "new") {
        ...
        if (managedUrl === null || !(await localStack.matchesDesiredStack())) {
          return {
            ok: false,
            error: "The app-managed Bot services are not ready. Retry setup.",
          };
        }
