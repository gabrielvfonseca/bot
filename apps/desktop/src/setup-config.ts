...
import type { DesktopSetup, DesktopStackProbeResponse } from "@bot/contracts";
...
/**
 * Accepts what a person would actually type ("localhost:5173", "bot.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a Bot server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // Bot serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `BOT_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Bot origin gets its own persistent cookie and storage partition. */
...
export function isBotHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Bot
