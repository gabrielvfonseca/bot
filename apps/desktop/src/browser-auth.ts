...
import type { BotDesktopOAuthCallback } from "@bot/contracts";
...
export async function openBrowserAuth(
  ...
  options: {
    ...
    onCallback: (callback: BotDesktopOAuthCallback) => void;
    onClose?: () => void;
  },
): Promise<void> {
      ...
      for (const host of hosts) {
        ...
        const server = createServer({ maxHeaderSize: 8192 }, (request, response) => {
          ...
          if (
            ...
            response.writeHead(400).end("Sign-in callback rejected. Return to Bot to retry.");
            return;
          }
          consumed = true;
          response.end("You can close this tab and return to Bot.", close);
