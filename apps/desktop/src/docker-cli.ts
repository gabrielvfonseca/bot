...
import type { DesktopSetupLink } from "@bot/contracts";
...
/**
 * `BOT_DOCKER_BINARY` is a test hook and the only candidate when set, so a
...
export function resolveDockerBinary(
  ...
  const override = env.BOT_DOCKER_BINARY?.trim();
