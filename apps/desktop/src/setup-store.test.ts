...
beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "bot-setup-"));
});
...
describe("setup store", () => {
  ...
  it("keeps the chosen instance across launches", async () => {
    await writeSetup(userData, { mode: "existing", serverUrl: "https://bot.example.com" });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await writeFile(
        target,
        JSON.stringify({ mode: "existing", serverUrl: "https://bot.example.com" }),
        "utf8",
      );
