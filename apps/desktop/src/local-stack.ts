...
import type { DesktopLocalStackState } from "@bot/contracts";
import {
...
export const STACK_PROJECT_NAME = "bot-desktop";
...
const LAUNCH_SUPPLIED = ["BOT_IMAGE_TAG", "BOT_COMPUTER_IMAGE_TAG"];
  ...
  switch (kind) {
    ...
    case "other":
      return phase === "pulling"
        ? "Downloading Bot images failed. Check the output below, then retry."
        : "Bot services did not start. Check the output below, then retry.";
  }
}
  ...
  private docker(binary: string, args: string[], timeoutMs: number, signal?: AbortSignal) {
    return this.deps.run(binary, args, {
      ...
      env: dockerSpawnEnv(this.deps.platform, this.deps.env, binary, {
        BOT_IMAGE_TAG: this.deps.imageTag,
        BOT_COMPUTER_IMAGE_TAG: this.deps.imageTag,
        ...(this.currentStackToken === null
          ? {}
          : { BOT_DESKTOP_STACK_TOKEN: this.currentStackToken }),
        BOT_WEB_PORT: new URL(this.currentWebUrl).port || "80",
