...
const nativeNotifications =
  Platform.OS === "android" ? requireNativeModule<NativeNotifications>("BotNotifications") : null;
...
export function notificationTargetsThread(
  ...
  const dataThreadId = data.threadId ?? data["bot.threadId"];
  if (target.threadId && dataThreadId) return dataThreadId === target.threadId;
  return Boolean(
    target.botId && (data.botId === target.botId || data["bot.botId"] === target.botId),
  );
}
