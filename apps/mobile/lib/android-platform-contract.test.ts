...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/bot-notifications/android/src/main/java/com/bot/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "BotNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "BotNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("BotNotificationService.clearSession(context)");
    ...
    expect(service).toContain('connection.setRequestProperty("x-bot-space-id", spaceId)');
    ...
    expect(service).toContain('"bot://group-thread?groupId=');
    expect(service).toMatch(/&spaceId=\$\{Uri\.encode\(run\.spaceId\)\}/);
    expect(service).toContain('putString("bot.spaceId", run.spaceId)');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/bot-notifications/android/src/main/java/com/bot/notifications/BotNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("bot.botId", run.botId)');
