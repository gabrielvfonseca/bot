...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://bot.example.com:8443/api/")).toEqual({
      ok: true,
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://bot.example.com")).toBe("bot.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://bot.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Bot /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Bot", async () => {
    const fetchImpl = vi.fn(
