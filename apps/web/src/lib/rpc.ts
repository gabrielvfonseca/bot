import type { AppContract } from "@bot/contracts";
import { LOCAL_SETTINGS_PAGE, LOCAL_SETTINGS_RPC } from "@bot/contracts";
import { createORPCClient } from "@orpc/client";
import { RPCLink } from "@orpc/client/fetch";
import type { ContractRouterClient } from "@orpc/contract";
import { desktopBridge } from "./desktop";

const SPACE_STORAGE_KEY = "bot:space-id";
...
/** Adds `x-bot-space-id` when a space is selected. */
export function withSpaceHeaders(
  init?: HeadersInit,
  spaceId: string | null = selectedSpaceId(),
): Headers {
  const headers = new Headers(init);
  if (spaceId) headers.set("x-bot-space-id", spaceId);
  else headers.delete("x-bot-space-id");
  return headers;
}
