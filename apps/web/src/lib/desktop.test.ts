...
import {
  ...
  type BotDesktop,
  type BotDesktopOAuthCallback,
  windowChromeKind,
} from "./desktop.js";

function desktop(platform: string): BotDesktop {
  const updateState = {
...
describe("attempt correlation", () => {
  ...
  function bridgeEmitting() {
    let emit: (callback: BotDesktopOAuthCallback) => void = () => undefined;
    const unsubscribe = vi.fn();
    vi.stubGlobal("window", {
      botDesktop: {
        ...desktop("linux"),
        oauth: {
          onCallback: (listener: (callback: BotDesktopOAuthCallback) => void) => {
            emit = listener;
            return unsubscribe;
          },
        },
    ...
    return { emit: (c: BotDesktopOAuthCallback) => emit(c), unsubscribe };
  }
