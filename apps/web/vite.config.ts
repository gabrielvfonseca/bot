...
export default defineConfig(({ mode }) => {
  ...
  const previewHost = process.env.BOT_HOST ?? rootEnv.BOT_HOST ?? "localhost";
  ...
  const performanceAssetDelayMs = Number(process.env.BOT_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  const desktopStackToken =
    process.env.BOT_DESKTOP_STACK_TOKEN ?? rootEnv.BOT_DESKTOP_STACK_TOKEN ?? "";
  const imageTag = process.env.BOT_IMAGE_TAG ?? rootEnv.BOT_IMAGE_TAG ?? "edge";
  return {
    plugins: [
      react(),
      ...
      {
        name: "bot-performance-asset-delay",
        configurePreviewServer(server) {
