...
test("restricted signup waits for mailbox verification", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Bot" })).toBeVisible();
});
...
test("logout protects bot deep links and sign-in restores the session", async ({
  ...
  const email = `auth-lifecycle-${stamp}@bot.test`;
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Bot" })).toBeVisible();
  await page.goto("/");
  await expect(page.locator('[data-bot-surface="welcome"]')).toBeVisible();
  await expect(page.getByText(/Your team of always-on agents/)).toBeVisible();
  await page.getByRole("button", { name: /Sign up/ }).click();
  await expect(page).toHaveURL(/\/sign-up$/);
  await expect(page.getByRole("heading", { name: "Create your Bot" })).toBeVisible();
  await page.goto("/");
...
test("changes and recovers an email password", async ({ page }, testInfo) => {
  const stamp = Date.now();
  const email = `password-recovery-${stamp}@bot.test`;
