...
test("create group from + and see two bots in one transcript", async ({ page }, testInfo) => {
  ...
  await signup(page, `group-${stamp}@bot.test`, "password12", "Group E2E");
  ...
  const groupAvatar = page
    ...
    .locator(".bot-group-avatar");
  await expect(groupAvatar).toBeVisible();
  await expect(groupAvatar.locator(".bot-bot-avatar")).toHaveCount(2);
  const workingAvatar = groupAvatar.locator('[data-working="true"]');
  await expect(workingAvatar).toHaveCount(1);
  await expect(workingAvatar.locator(".bot-bot-avatar-ring")).toHaveCSS(
    "animation-name",
    "bot-avatar-spin",
  );
