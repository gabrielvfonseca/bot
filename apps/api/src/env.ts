...
export { resolveCloudAgentProvider, resolveSandboxProvider } from "@bot/adapters";
...
export function loadEnv(source: NodeJS.ProcessEnv = process.env): AppEnv {
  ...
  const updaterUrl = optional(source.BOT_UPDATER_URL);
  const updaterToken = optional(source.BOT_UPDATER_TOKEN);
  return {
    nodeEnv: source.NODE_ENV ?? "",
    databaseUrl: required(source, "DATABASE_URL"),
    realtimeDatabaseUrl: source.REALTIME_DATABASE_URL ?? required(source, "DATABASE_URL"),
    desktopStackToken: optional(source.BOT_DESKTOP_STACK_TOKEN),
    authSecret,
    ...
    teamChatBotId: optional(source.TEAM_CHAT_BOT_ID) ?? optional(source.SLACK_BOT_BOT_ID),
    ...
    gitSha: optional(source.GIT_SHA) ?? optional(source.BOT_GIT_SHA),
    updaterUrl,
    updaterToken,
    imageTag: optional(source.BOT_IMAGE_TAG),
  };
}
