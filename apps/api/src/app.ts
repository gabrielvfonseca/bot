...
export async function createApp(
  ...
  app.use("/rpc/*", async (c, next) => {
    ...
    const requestedSpaceId = c.req.header("x-bot-space-id");
...
function isTrustedOrigin(origin: string, env: AppEnv) {
  ...
  if (origin.startsWith("bot://") || origin.startsWith("exp://")) return true;
  try {
