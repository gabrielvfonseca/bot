...
import { ServerUpdateRunSchema } from "@bot/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `BOT_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.BOT_IMAGE_TAG?.trim() || null;
