...
describe("server update install kind", () => {
  async function tempRoot(withGit: boolean) {
    const root = await mkdtemp(path.join(tmpdir(), "bot-update-"));
    roots.push(root);
        ...
        return new Response(
          JSON.stringify({
            ...
            checkout: {
              ...
              branch: "main",
              remoteUrl: "https://github.com/elie222/bot",
              dirty: false,
              dirtyPaths: [],
            },
          }),
...
describe("sidecar proxy auth and no-git-apply", () => {
  ...
  it("sends the bearer token to the sidecar and never returns it", async () => {
    ...
    const root = await mkdtemp(path.join(tmpdir(), "bot-proxy-"));
    roots.push(root);
