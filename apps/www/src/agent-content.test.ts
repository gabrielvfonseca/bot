...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# Bot");
    expect(getMarkdownDocument("/about/")).toContain("# About Bot");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use Bot");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use Bot");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# Bot\n");
    ...
    await expect(response.text()).resolves.toBe("# Bot\n");

    const headResponse = markdownResponse("# Bot\n", "HEAD", 404);
