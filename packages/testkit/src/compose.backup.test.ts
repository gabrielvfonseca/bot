...
function postgresUp() {
  try {
    execSync(`docker compose -f ${composeFile} exec -T postgres pg_isready -U bot`, {
      stdio: "ignore",
      timeout: 8_000,
    });
    return true;
...
describeBackup("compose backup and restore", () => {
  it("dumps postgres and restores into a side database", () => {
    ...
    execSync(`${backupScript} ${stamp}`, {
      stdio: "pipe",
      timeout: 60_000,
      env: { ...process.env, BOT_BACKUP_SKIP_HOMES: "1" },
    });
    const dump = path.resolve("backups", stamp, "bot.sql");
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U bot -d postgres -c "DROP DATABASE IF EXISTS bot_restore_test"`,
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U bot -d postgres -c "DROP DATABASE bot_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
