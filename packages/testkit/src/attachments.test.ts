...
import type { ThreadSnapshot } from "@bot/contracts";
...
describeAttachments("chat attachments", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "bot-attachments-"));
  ...
  it("uploads, sends with image/file, and rejects invalid attachments", async () => {
    const cookie = await signup(app, `attachments-${stamp}@bot.test`, "Attachment User");
  ...
  it("attaches a workspace file into the thread", async () => {
    const cookie = await signup(app, `attach-thread-${stamp}@bot.test`, "Attach Thread User");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "Attacher",
      title: "Attacher",
