...
const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "bot-desktop-performance-"));
...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "bot-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "bot-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "bot-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "bot-benchmark-screen-proxy-secret-over-32-characters",
    ...
    BOT_HOST: "127.0.0.1",
    BOT_WEB_URL: webOrigin,
    BOT_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    BOT_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
    BOT_PERFORMANCE_ASSET_DELAY_MS: String(assetDelayMs),
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@bot/desktop", "pack:dir"], env);
}
...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "Bot.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/Bot");
  }
...
async function prepareAuthenticatedProfile(benchmark: BenchmarkContext, profile: string) {
  ...
  try {
    ...
    await page.getByPlaceholder("Your email address").fill(`benchmark-${stamp}@bot.test`);
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      BOT_PERFORMANCE_USER_DATA: profile,
      BOT_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "Bot.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# Bot desktop performance — ${report.label}
