...
import type { RunsListOutput, ThreadSnapshot } from "@bot/contracts";
import { isTerminal } from "@bot/core";
...
const project = `bot-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "bot/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "bot/computer:topology",
  };
...
async function signup(baseUrl: string) {
  const response = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({
      email: `topology-${Date.now()}@bot.test`,
      password: "password12",
      name: "Topology",
    }),
  });
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // {"timestamp":...,"level":"info","message":"worker ready","service.name":"bot-worker"},
    // not the old plain "bot worker ready" console.log line -- match the JSON field directly
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "bot.managed"}}',
  ]).trim();
