...
import { ComposioEmulator, FakeSandboxProvider } from "@bot/adapters";
...
describe.skipIf(!databaseAvailable)("offline Pi computer approval", () => {
  beforeAll(() => {
    ...
    vi.stubEnv("BOT_OPENAI_COMPATIBLE_VISION_MODELS", "offline-fixture");
  });
  ...
  it.each(["allow", "deny"] as const)(
    ...
    async (answer) => {
      ...
      const dataDir = await mkdtemp(path.join(tmpdir(), "bot-computer-approval-"));
      ...
      try {
        ...
        const signup = await handles.app.request("/api/auth/sign-up/email", {
          ...
          body: JSON.stringify({
            email: `computer-approval-${randomUUID()}@bot.test`,
            password: "password12",
