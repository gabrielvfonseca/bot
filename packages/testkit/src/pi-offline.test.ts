...
import type { AgentRunRequest, AgentRuntimeEvent, ConnectorTool } from "@bot/adapter-kit";
import { PiAgentRuntime } from "@bot/adapters";
...
describe("real Pi against an offline model HTTP endpoint", () => {
  it("assembles fragmented tool arguments, executes the write, and sends its result back", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "bot-pi-offline-"));
