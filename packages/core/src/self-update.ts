/** The repository a stock Bot deployment tracks. */
export const OFFICIAL_REPO_URL = "https://github.com/elie222/bot";
...
/** `github.com/elie222/bot` for every spelling of the same remote, for comparison only. */
...
export const RESTART_SUPERVISOR_ENV = "BOT_UPDATE_RESTART_SUPERVISOR";
...
export function restartSupervisorAdvice(supervisor: RestartSupervisor): string {
  if (supervisor.kind !== "none") {
    return `Bot will exit after updating and ${supervisor.label} will start it on the new code.`;
  }
  return `No process supervisor was detected, so Bot will not exit on its own. Restart the API, worker, and web processes yourself, or set ${RESTART_SUPERVISOR_ENV} to the supervisor that restarts them (for example "docker" with restart: unless-stopped, or run under systemd with Restart=always).`;
}
