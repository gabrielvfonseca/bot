...
describe("secrets-guard", () => {
  ...
  it("requires the updater to use a dedicated token", () => {
    ...
    ).toThrow(/BOT_UPDATER_TOKEN/);
    expect(
      resolveUpdaterToken({
        NODE_ENV: "test",
        BOT_UPDATER_TOKEN: "updater-only",
        SANDBOX_SUPERVISOR_TOKEN: "supervisor-only",
        BETTER_AUTH_SECRET: "custom-auth",
      }),
    ).toBe("updater-only");
    ...
    expect(() =>
      resolveUpdaterToken({
        NODE_ENV: "test",
        BOT_UPDATER_TOKEN: "supervisor-only",
        SANDBOX_SUPERVISOR_TOKEN: "supervisor-only",
      }),
    ).toThrow(/must differ/);
    expect(() =>
