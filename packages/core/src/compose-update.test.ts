...
const target = {
  composeFiles: ["/srv/bot/infra/compose/docker-compose.prod.yml"],
  envFiles: ["/srv/bot/.env"],
};
...
describe("image references", () => {
  ...
  it("accepts registry paths with a host and rejects anything else", () => {
    ...
    expect(isValidImageName("bot/app")).toBe(true);
...
describe("compose argv construction", () => {
  ...
  it("passes one --file per Compose file, in overlay order", () => {
    const overlaid = {
      ...target,
      composeFiles: [
        "/srv/bot/infra/compose/docker-compose.prod.yml",
        "/srv/bot/ops/compose/docker-compose.sandbox.yml",
      ],
    };
  ...
  it("defaults -p to the name pinned in the compose file", () => {
    expect(DEFAULT_COMPOSE_PROJECT_NAME).toBe("bot-prod");
    expect(composePsArgv(target).args.slice(0, 3)).toEqual(["compose", "-p", "bot-prod"]);
  });
  ...
  it("prefers the name Compose injects into running services", () => {
    ...
    expect(
      resolveComposeProjectName({
        ...
        BOT_COMPOSE_PROJECT_NAME: "manual-stack",
      }),
    ).toBe("live-stack");
  });

  it("falls back to a dedicated override, then to the name pinned in the compose file", () => {
    expect(resolveComposeProjectName({ BOT_COMPOSE_PROJECT_NAME: "manual-stack" })).toBe(
      "manual-stack",
    );
...
describe("update plans", () => {
  ...
  it("fetches, fast-forwards, then builds on the fork path", () => {
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl: "https://github.com/someone/bot",
      branch: "trunk",
      repointRemote: true,
    });
  ...
  it("keeps the repository URL in argv rather than in any compose input", () => {
    const repoUrl = "https://github.com/someone/bot";
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl,
...
describe("managed env assignments", () => {
  ...
  it("refuses values that would inject a second assignment or a compose expression", () => {
    ...
    for (const value of ["v1\nEVIL=1", "v1 v2", "$(id)", "v1;rm -rf /", "${OTHER}"]) {
      expect(() => upsertEnvAssignments("", { BOT_IMAGE_TAG: value })).toThrow(/Refusing/);
    }
  });
...
describe("sidecar boundary validation", () => {
  it("normalizes a request the API already validated", () => {
    expect(
      validateUpdateRequest({ repoUrl: "https://github.com/elie222/bot.git", branch: " main " }),
    ).toEqual({
      request: {
        repoUrl: "https://github.com/elie222/bot.git",
        branch: "main",
        official: true,
      },
    });
