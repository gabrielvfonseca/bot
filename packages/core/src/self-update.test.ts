...
describe("normalizeRepoUrl", () => {
  it("accepts the official repository and https forks", () => {
    expect(normalizeRepoUrl(OFFICIAL_REPO_URL)).toEqual({
      url: "https://github.com/elie222/bot",
    });
    expect(normalizeRepoUrl("  https://github.com/me/bot.git/  ")).toEqual({
      url: "https://github.com/me/bot.git",
    });
    expect(normalizeRepoUrl("https://git.example.com:8443/team/bot.git")).toEqual({
      url: "https://git.example.com:8443/team/bot.git",
    });
  });

  it("accepts ssh remotes in both spellings", () => {
    expect(normalizeRepoUrl("git@github.com:me/bot.git")).toEqual({
      url: "git@github.com:me/bot.git",
    });
    expect(normalizeRepoUrl("ssh://git@github.com/me/bot.git")).toEqual({
      url: "ssh://git@github.com/me/bot.git",
    });
  });
  ...
  it("refuses transports and shapes that are not a git remote", () => {
    for (const input of [
      "",
      "   ",
      "http://github.com/me/bot",
      "git://github.com/me/bot",
      "file:///etc/passwd",
      "/srv/bot",
      "https://github.com",
      "https://github.com/onlyone",
      "https://github.com/me/../../etc",
      "https://user:secret@github.com/me/bot",
      "https://token@github.com/me/bot",
      "ssh://bad%0auser@github.com/me/bot",
      "https://github.com/me/bot?x=1",
      "https://github.com/me/bot#frag",
      "https://github.com/me/rak azo",
      "--upload-pack=touch /tmp/pwned",
      "-o ProxyCommand=id",
      `https://github.com/me/bot${"x".repeat(400)}`,
    ]) {
  ...
  it("rejects an embedded newline that could smuggle a second argument", () => {
    expect(normalizeRepoUrl("https://github.com/me/bot\n--exec=id")).toHaveProperty("error");
  });
});

describe("repoIdentity", () => {
  it("treats every spelling of the same remote as one repository", () => {
    const identity = "github.com/elie222/bot";
    expect(repoIdentity("https://github.com/elie222/bot")).toBe(identity);
    expect(repoIdentity("https://github.com/elie222/bot.git")).toBe(identity);
    expect(repoIdentity("git@github.com:elie222/bot.git")).toBe(identity);
    expect(repoIdentity("ssh://git@github.com/Elie222/Bot")).toBe(identity);
    expect(repoIdentity("not a url")).toBeNull();
  });

  it("only calls the real upstream official", () => {
    expect(isOfficialRepoUrl("git@github.com:elie222/bot.git")).toBe(true);
    expect(isOfficialRepoUrl("https://github.com/attacker/bot")).toBe(false);
    expect(isOfficialRepoUrl("https://githubb.com/elie222/bot")).toBe(false);
  });
});
...
describe("updateSteps", () => {
  ...
  it("passes every value as its own argument so nothing reaches a shell", () => {
    const steps = updateSteps({
      remoteUrl: "git@github.com:me/bot.git",
      branch: "release/1.2",
      targetCommit: "2".repeat(40),
      repointRemote: true,
    });
    expect(steps[0]).toEqual({
      ...
      command: "git",
      args: ["remote", "set-url", "origin", "git@github.com:me/bot.git"],
    });
