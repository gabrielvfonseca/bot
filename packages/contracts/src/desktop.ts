...
export interface BotDesktopUpdate {
...
export interface BotDesktopOAuthCallback {
  code: string;
  state?: string;
}
...
export interface BotDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: BotDesktopOAuthCallback) => void) => () => void;
  };
}

/**
 * How the desktop app was pointed at a Bot server during first-run setup.
 ...
 * narrower `botDesktop` bridge so a connected server can never re-point the app.
 */
