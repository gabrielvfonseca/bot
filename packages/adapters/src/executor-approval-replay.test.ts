...
describe("executor approval replay", () => {
  ...
  it("replays an approved lazy tool through the exposed catalog executor", () => {
    ...
    expect(continuation).not.toContain("__botCatalogTool");
  });
  ...
  it("keeps a direct-tool argument named like the catalog marker in continuation JSON", () => {
      ...
      [
        {
          ...
          request: {
            ...
            __botCatalogTool: "installed_execute_tool",
          },
        },
      ],
      JSON.stringify,
    ...
    expect(continuation).toContain(
      'notes.write: {"id":"row-1","arguments":{"mode":"strict"},"text":"approved exactly","__botCatalogTool":"installed_execute_tool"}',
    );
  });
  ...
  it("preserves direct approved args that use the catalog marker as data", () => {
    const approvedRequest = {
      __botCatalogTool: "user-provided-value",
      target: "approved",
    };
    ...
    expect(
      approvedReplayArgs(
        approvedRequest,
        { __botCatalogTool: "user-provided-value", target: "reconstructed" },
        "__botCatalogTool",
      ),
