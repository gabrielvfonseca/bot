import type { BackgroundJobHandlers } from "@bot/adapter-kit";
...
describe("InMemoryJobQueue", () => {
  ...
  it("keeps wrapped payloads readable by workers that do not unwrap envelopes", async () => {
    const { wrapJobPayload } = await import("@bot/logging");
    const { parseBackgroundJob } = await import("@bot/adapter-kit");
