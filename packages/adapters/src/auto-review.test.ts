...
describe("resolveAutoReviewChecker", () => {
  it("prefers explicit env overrides", () => {
    expect(
      resolveAutoReviewChecker({
        BOT_AUTO_REVIEW_PROVIDER: "openrouter",
        BOT_AUTO_REVIEW_MODEL: "cheap/fast",
        BOT_LOCAL_MODELS: "local-a",
  ...
  it("prefers local models when configured", () => {
    expect(
      resolveAutoReviewChecker({
        BOT_LOCAL_MODELS: " llama-local , other ",
...
describe("deploymentAutoReviewDefault", () => {
  it("defaults off", () => {
    ...
    expect(deploymentAutoReviewDefault({ BOT_AUTO_REVIEW: "1" })).toBe(true);
    expect(deploymentAutoReviewDefault({ BOT_AUTO_REVIEW: "true" })).toBe(true);
  });
});
...
describe("autoReviewTimeoutMs", () => {
  it("defaults to 1500 and clamps bad values", () => {
    ...
    expect(autoReviewTimeoutMs({ BOT_AUTO_REVIEW_TIMEOUT_MS: "2000" })).toBe(2_000);
    expect(autoReviewTimeoutMs({ BOT_AUTO_REVIEW_TIMEOUT_MS: "nope" })).toBe(1_500);
  });
});
