...
describe("SerenityMemoryProvider", () => {
  it("keeps bot and space entity namespaces inside the adapter", () => {
    expect(serenityBotEntity("bot-1")).toBe("bot-bot/bot-1");
    expect(serenitySpaceEntity("workspace-1")).toBe("bot-space/workspace-1");
    expect(serenityBotEntity("bot-1", "Personal Brain")).toBe(`bot-bot/${PERSONAL_BRAIN}/bot-1`);
    expect(serenitySpaceEntity("workspace-1", "Personal Brain")).toBe(
      `bot-space/${PERSONAL_BRAIN}/workspace-1`,
    );
  });
  ...
  it("recalls isolated scope only against the bot entity", async () => {
    ...
    expect(recallSerenityMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ entity: "bot-bot/bot-1", limit: 5 }),
    );
  });
  ...
  it("forgets by fact id only (Serenity forget has no entity argument)", async () => {
    ...
    await labeled.forget(
      { id: "fact-9", entity: `bot-bot/${PERSONAL_BRAIN}/bot-1`, reason: "cleanup" },
      context,
    );
      ...
      value: [
        {
          ...
          id: "fact-space-1",
          provenance: "space policy",
          entity: `bot-space/${PERSONAL_BRAIN}/workspace-1`,
        },
      ],
    });
