import type { AdapterContext, ConnectorEvent, ConnectorTool } from "@bot/adapter-kit";
...
describe("composio tool mapping", () => {
  ...
  it("logs sanitized connector discovery failures", async () => {
    ...
    installLogger(createLogger({ service: "bot-api", sinks: [sink] }));
    ...
    try {
      ...
      installLogger(createLogger({ service: "bot-api", level: "off", sinks: [] }));
    }
  });
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__bot_catalog__", toolkits: undefined, sessionPreset: undefined },
  ...
  it("only fetches live Composio slugs when a Bot row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but Bot is still pending", () => {
    expect(
      planLiveConnectionSync(
        [
