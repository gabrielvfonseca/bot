...
describe("E2B computer backend", () => {
  it("revokes an extra display's control without starting or waiting for its view", async () => {
    const command = vi.fn(async (value: string) => {
      if (value.includes("BOT_SCREEN_INDEX=")) {
        return { stdout: "BOT_SCREEN_INDEX=1\n", stderr: "", exitCode: 0 };
      }
      if (value.includes("BOT_SCREEN_PASSWORD=") || value.includes("flock 8")) {
  ...
  it("opens http(s) URLs through the named browser launcher", async () => {
    ...
    expect(command).toHaveBeenLastCalledWith(
      expect.stringContaining("/home/user/bot-home/notes/readme.md"),
      expect.anything(),
    );
  });
...
describe("sandbox-gone detection", () => {
  ...
  const alive = [
    ...
    new Error("Path /home/user/bot-home/notes.md not found"),
