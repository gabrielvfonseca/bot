...
describe("Pi JSONL sessions", () => {
  ...
  it("uses Pi's session format for context and completed messages", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "bot-pi-session-"));
    try {
      ...
      expect(records).toContainEqual(
        expect.objectContaining({
          kind: "entry",
          customType: "bot_context",
          data: expect.objectContaining({
            botThreadId: "thread-1",
            botTraceId: "trace-1",
          }),
        }),
      );
  ...
  it("keeps sessions scoped and bounded by the retention policy", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "bot-pi-retention-"));
    try {
  ...
  it("removes bot and account session scopes", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "bot-pi-cleanup-"));
    try {
