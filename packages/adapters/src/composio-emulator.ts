    ...
    messages: [
      {
        ...
        subject: "Welcome to Bot",
        sender: "hello@bot.test",
        to: "me@example.test",
  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          repo: { type: "string", description: "Repository name, e.g. bot" },
        },
        required: ["owner", "repo"],
      },
    },
...
export class ComposioEmulator implements ComposioProvider {
  ...
  private githubReleases: EmulatedGithubRelease[] = [...DEFAULT_BOT_EMULATED_RELEASES];
  ...
  private executeGithub(tool: string, args: Record<string, unknown>): Record<string, unknown> {
    ...
    const repo = String(args.repo ?? args.repository ?? "bot");
