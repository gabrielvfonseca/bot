import type { ComputerAction, ComputerInput } from "@bot/adapter-kit";
...
export function observeExtraDisplayCommand(layout: ExtraDisplayLayout): string {
  const imagePath = `/tmp/bot/observe-${layout.displayNumber}.png`;
  return [
    `DISPLAY=${layout.display} xdotool getmouselocation --shell >/tmp/bot/cursor-${layout.displayNumber}.txt || true`,
    ...
    `printf '\\nCURSOR '`,
    `tr '\\n' ' ' </tmp/bot/cursor-${layout.displayNumber}.txt 2>/dev/null || true`,
  ].join("; ");
}
