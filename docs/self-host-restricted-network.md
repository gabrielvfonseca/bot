...
| Cannot fetch Compose files | `BOT_DOWNLOAD_BASE`, `--local`, or `BOT_DOWNLOAD_SKIP_EXISTING=1` |
| Cannot pull app or computer images | `BOT_IMAGE`, `BOT_IMAGE_TAG`, `BOT_COMPUTER_IMAGE`, `BOT_COMPUTER_IMAGE_TAG` |
...
```bash
export BOT_INSTALLER_URL=https://example.com/mirror/bot/infra/compose/install-images.sh
export BOT_DOWNLOAD_BASE=https://example.com/mirror/bot/infra/compose
mkdir -p bot && cd bot &&
curl -fsSL -o install-images.sh "${BOT_INSTALLER_URL}" &&
bash install-images.sh --prepare-only
```

`BOT_INSTALLER_URL` selects the script for this curl command; `BOT_DOWNLOAD_BASE` selects
...
BOT_DOWNLOAD_SKIP_EXISTING=1 bash install-images.sh --prepare-only
```
...
```env
BOT_IMAGE=registry.example.com/mirror/bot/app
BOT_IMAGE_TAG=edge
BOT_COMPUTER_IMAGE=registry.example.com/mirror/bot/computer
BOT_COMPUTER_IMAGE_TAG=edge
