import { DEFAULT_COMPOSE_PROJECT_NAME, isLocalImageTag } from "@bot/core";
...
const base = {
  ...
  BOT_UPDATER_TOKEN: "fake-review-updater-token-000000000000",
} as const;
...
describe("resolveUpdaterConfig", () => {
  it("derives the compose file, env file, and defaults from the deployment directory", () => {
    ...
    expect(config).toMatchObject({
      deployDir: "/srv/bot",
      composeFiles: [`/srv/bot/${DEFAULT_COMPOSE_FILE}`],
      updateServices: ["api", "worker", "web"],
      envFile: "/srv/bot/.env",
      projectName: DEFAULT_COMPOSE_PROJECT_NAME,
      token: base.BOT_UPDATER_TOKEN,
  ...
  it("uses the Compose-injected project name so -p matches the running stack", () => {
    ...
    expect(
      resolveUpdaterConfig({ ...base, BOT_COMPOSE_PROJECT_NAME: "manual-stack" }).projectName,
    ).toBe("manual-stack");
  });
  ...
  it("binds to loopback unless the deployment says otherwise, so a stray port is not a door", () => {
    ...
    expect(resolveUpdaterConfig({ ...base, BOT_UPDATER_HOST: "0.0.0.0" }).host).toBe("0.0.0.0");
  });

  it("refuses a deployment directory that is missing or relative", () => {
    expect(() => resolveUpdaterConfig({ BOT_UPDATER_TOKEN: "t" })).toThrow(/BOT_DEPLOY_DIR/);
    expect(() => resolveUpdaterConfig({ ...base, BOT_DEPLOY_DIR: "srv/bot" })).toThrow(
      /BOT_DEPLOY_DIR/,
    );
  });
  ...
  it("refuses a compose path that escapes the deployment directory", () => {
    for (const composeFile of ["/etc/compose.yml", "../../etc/compose.yml", "a/../../b.yml"]) {
      expect(() => resolveUpdaterConfig({ ...base, BOT_COMPOSE_FILE: composeFile })).toThrow(
        /BOT_COMPOSE_FILE/,
      );
    }
  });
  ...
  it("resolves a Compose file list in order, against the deployment directory", () => {
    expect(
      resolveUpdaterConfig({
        ...base,
        BOT_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:ops/overlay.yml",
      }).composeFiles,
    ).toEqual(["/srv/bot/infra/compose/docker-compose.prod.yml", "/srv/bot/ops/overlay.yml"]);
  });
  ...
  it("honours COMPOSE_PATH_SEPARATOR the way Compose does", () => {
    expect(
      resolveUpdaterConfig({
        ...
        BOT_COMPOSE_FILE: "a.yml,b.yml",
      }).composeFiles,
    ).toEqual(["/srv/bot/a.yml", "/srv/bot/b.yml"]);
  });
  ...
  it("checks every entry in a list, not just the first", () => {
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        BOT_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:../../etc/compose.yml",
      }),
    ).toThrow(/BOT_COMPOSE_FILE/);
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        BOT_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:/etc/compose.yml",
  ...
  it("refuses a Compose file list that names nothing", () => {
    expect(() => resolveUpdaterConfig({ ...base, BOT_COMPOSE_FILE: ":  :" })).toThrow(
      /BOT_COMPOSE_FILE/,
    );
  });
  ...
  it("appends the deployment's extra services to the built-in set", () => {
    expect(
      resolveUpdaterConfig({ ...base, BOT_UPDATE_SERVICES: "supervisor, caddy" }).updateServices,
    ).toEqual(["api", "worker", "web", "supervisor", "caddy"]);
  });

  it("cannot drop a built-in service, however BOT_UPDATE_SERVICES is written", () => {
    const services = resolveUpdaterConfig({
      ...base,
      BOT_UPDATE_SERVICES: "web,supervisor",
    }).updateServices;
  ...
  it("refuses to recreate the updater, which would kill the run mid-flight", () => {
    expect(() =>
      resolveUpdaterConfig({ ...base, BOT_UPDATE_SERVICES: "supervisor,updater" }),
    ).toThrow(/updater/);
  });
  ...
  it("refuses a service name it would not hand to compose as one argument", () => {
    for (const service of ["--build", "a b", "-f", "api;rm"]) {
      expect(() =>
        resolveUpdaterConfig({ ...base, BOT_UPDATE_SERVICES: `supervisor,${service}` }),
      ).toThrow(/BOT_UPDATE_SERVICES/);
    }
  });
  ...
  it("refuses a port that is not a port", () => {
    expect(() => resolveUpdaterConfig({ ...base, BOT_UPDATER_PORT: "0" })).toThrow(/port/);
    expect(() => resolveUpdaterConfig({ ...base, BOT_UPDATER_PORT: "seven" })).toThrow(/port/);
  });
});

describe("readEnvAssignment", () => {
  it("reads the last assignment, ignoring comments and blank lines", () => {
    const contents = ["# BOT_IMAGE_TAG=commented", "", "A=1", "A=2"].join("\n");
    expect(readEnvAssignment(contents, "A")).toBe("2");
    expect(readEnvAssignment(contents, "BOT_IMAGE_TAG")).toBeNull();
  });
  ...
  it("does not match a key that merely shares a prefix", () => {
    expect(readEnvAssignment("BOT_IMAGE_TAG_PREVIOUS=v1", "BOT_IMAGE_TAG")).toBeNull();
  });
});

describe("readTagState", () => {
  it("reads the pinned tag and the rollback tag", () => {
    const contents = "BOT_IMAGE_TAG=v1.1.0\nBOT_IMAGE_TAG_PREVIOUS=v1.0.0\n";
  ...
  it("ignores values in the file that are not usable tags", () => {
    const contents = "BOT_IMAGE_TAG=-rm\nBOT_IMAGE_TAG_PREVIOUS=$(id)\n";
