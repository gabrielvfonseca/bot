#!/usr/bin/env python3
"""Token-auth desktop control for the Bot supervisor."""
...
TOKEN = os.environ.get("BOT_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libbot-xcapture.so")
        library.bot_xcapture_open.argtypes = [ctypes.c_char_p]
        library.bot_xcapture_open.restype = ctypes.c_void_p
        library.bot_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.bot_xcapture_png.restype = ctypes.c_int
        library.bot_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.bot_xcapture_damage.restype = ctypes.c_int
        library.bot_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.bot_xinput_argv.restype = ctypes.c_int
        context = library.bot_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.bot_xcapture_png(
        ...
        changed = self.library.bot_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.bot_xinput_argv(self.context, len(argv), encoded)
...
def control_command_index(argv):
    ...
    return 3 if len(argv) > 2 and argv[2].startswith("BOT_BROWSER_PROFILE=") else 2
...
def allowed_control_argv(argv, display):
    ...
    command = argv[index]
    if index == 3:
        profile = argv[2].removeprefix("BOT_BROWSER_PROFILE=")
        if not re.fullmatch(r"/home/bot/\.browser-profiles/chromium-bot-[a-f0-9]{32}", profile):
            return False
        if command not in ("xdg-open", "bot-browser"):
            return False
    if command == "xdotool":
