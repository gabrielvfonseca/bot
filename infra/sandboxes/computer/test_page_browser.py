...
loader = importlib.machinery.SourceFileLoader("page_browser", str(Path(__file__).with_name("bot-page-browser")))
...
class PageBrowserTest(unittest.TestCase):
    def test_closed_stdin_cancels_helper_without_a_browser(self):
        with subprocess.Popen([sys.executable, str(Path(__file__).with_name("bot-page-browser")), "snapshot", "{}"],
                              env={**os.environ, "BOT_BROWSER_WATCH_STDIN": "1", "BOT_CDP_PORT": "0"},
