...
} from "@bot/core/node/desktop-runtime";
...
export { BROWSER_APPLICATIONS as DOCKER_BROWSER_ALIASES } from "@bot/core/node/desktop-runtime";
...
export function hasComputerIdentity(
  ...
  const labeledSpaceId = labels?.["bot.spaceId"] ?? labels?.["bot.workspaceId"];
  return labels?.["bot.botId"] === botId && labeledSpaceId === spaceId;
}
...
export function containerActionStep(
  ...
  if (action.kind === "key" || action.kind === "pointer" || action.kind === "clipboard") {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browserProfile ? [`BOT_BROWSER_PROFILE=${browserProfile}`] : []),
      "xdg-open",
      target,
    ];
  } else {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browser && browserProfile ? [`BOT_BROWSER_PROFILE=${browserProfile}`] : []),
      browser ? "bot-browser" : action.application,
...
export function workspaceTarget(relative: string) {
  return relative ? path.posix.join("/home/bot", relative) : "/home/bot";
}
