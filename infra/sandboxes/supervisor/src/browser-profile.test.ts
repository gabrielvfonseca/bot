...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "bot-profile-test-"));
  fixtures.push(root);
  ...
  mkdirSync(runtime);
  const profile = (bot: string) => browserProfilePathForScreen(bot).replace("/home/bot", home);
  const run = (script: string) => {
    ...
    return spawnSync(
      "bash",
      ["-eu", "-c", portable.replaceAll("/tmp/bot", runtime).replaceAll("/home/bot", home)],
      {
        encoding: "utf8",
        timeout: 10_000,
      },
