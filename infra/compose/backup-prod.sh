...
PROJECT_DIR="${BOT_DEPLOY_DIR:-/srv/bot}"
[[ "${PROJECT_DIR}" == /* ]] || { echo "BOT_DEPLOY_DIR must be an absolute path" >&2; exit 1; }
COMPOSE_FILE="${PROJECT_DIR}/infra/compose/docker-compose.prod.yml"
ENV_FILE="${PROJECT_DIR}/.env"
BACKUP_ROOT="/var/backups/bot"
...
"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/bot.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/bot.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified Bot backup written to ${SNAPSHOT_DIR}"
